...
// Use ConfigFactory inside TransparentUPgradableProxy for generation of config contracts
async function main() {
  ...
  const Proxy = await ethers.getContractFactory('TransparentUpgradeableFactoryProxy');
  const proxy = await Proxy.deploy(admin.address, configFactory.address, 100000, 200000);
  ...
  const upgradeTx = await proxy.connect(admin).upgradeToParams(300000, 400000);
