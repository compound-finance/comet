...
class Loader<T> {
  ...
  addScenario(name: string, requirements: object, property: Property<T>, initializer: Initializer<T>, forker: Forker<T>, constraints: Constraint<T>[], flags: ScenarioFlags = null) {
    if (this.scenarios[name]) {
      throw new Error(`Duplicate scenarios by name: ${name}`);
    }
    this.scenarios[name] = new Scenario<T>(name, requirements, property, initializer, forker, constraints, flags);
  }
