...
import { Scenario, Property, Initializer, Forker } from './Scenario';
...
class Loader<T> {
  ...
  addScenario(name: string, requirements: object, property: Property<T>, initializer: Initializer<T>, forker: Forker<T>) {
    if (this.scenarios[name]) {
      throw new Error(`Duplicate scenarios by name: ${name}`);
    }
    this.scenarios[name] = new Scenario<T>(name, requirements, property, initializer, forker);
  }
