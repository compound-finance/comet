...
type ScenarioFn<T, U> = (name: string, requirements: object, property: Property<T, U>) => Promise<void>;
...
export function addScenario<T, U>(
  ...
  getLoader().addScenario(name, requirements, property, initializer, transformer, forker, constraints, flags);
}
...
export function buildScenarioFn<T, U>(
  ...
  let addScenarioWithOpts =
    (flags: ScenarioFlags) => (name: string, requirements: object, property: Property<T, U>) => {
      addScenario<T, U>(name, requirements, property, initializer, transformer, forker, constraints, flags);
    };
