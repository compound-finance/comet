...
contract Comet is CometMath, CometStorage {
    ...
    function accrue() public {
        TotalsBasic memory totalsBasic_ = accrueInternal();
        baseSupplyIndex = totalsBasic_.baseSupplyIndex;
        baseBorrowIndex = totalsBasic_.baseBorrowIndex;
        trackingSupplyIndex = totalsBasic_.trackingSupplyIndex;
        trackingBorrowIndex = totalsBasic_.trackingBorrowIndex;
        totalSupplyBase = totalsBasic_.totalSupplyBase;
        totalBorrowBase = totalsBasic_.totalBorrowBase;
        lastAccrualTime = totalsBasic_.lastAccrualTime;
        pauseFlags = totalsBasic_.pauseFlags;
    }

    /**
     * @notice Accrue interest (and rewards) in base token supply and borrows
     **/
    function accrueInternal() internal view returns (TotalsBasic memory) {
        uint40 now_ = getNow();
        uint timeElapsed = now_ - lastAccrualTime;
        TotalsBasic memory totals;
        if (timeElapsed > 0) {
            uint supplyRate = getSupplyRateInternal();
            uint borrowRate = getBorrowRateInternal();
            totals.baseSupplyIndex += safe64(mulFactor(baseSupplyIndex, supplyRate * timeElapsed));
            totals.baseBorrowIndex += safe64(mulFactor(baseBorrowIndex, borrowRate * timeElapsed));
            if (totals.totalSupplyBase >= baseMinForRewards) {
                uint supplySpeed = baseTrackingSupplySpeed;
                totals.trackingSupplyIndex += safe64(divBaseWei(supplySpeed * timeElapsed, totalSupplyBase));
            }
            if (totals.totalBorrowBase >= baseMinForRewards) {
                uint borrowSpeed = baseTrackingBorrowSpeed;
                totals.trackingBorrowIndex += safe64(divBaseWei(borrowSpeed * timeElapsed, totalBorrowBase));
            }
        }
    ...
    function supplyBase(address from, address dst, uint104 amount) internal {
        ...
        totals.totalSupplyBase = principalValueSupply(totalSupplyBalance);
        totals.totalBorrowBase = principalValueBorrow(totalBorrowBalance);
        // XXX
        // totalsBasic = totals;

        updateBaseBalance(totals, dst, dstUser, principalValue(dstBalance));
    }
    ...
    function transferBase(address src, address dst, uint104 amount) internal {
        ...
        updateBaseBalance(totals, src, srcUser, principalValue(srcBalance));
        updateBaseBalance(totals, dst, dstUser, principalValue(dstBalance));
    ...
    function absorbInternal(address account) internal {
        ...
        updateBaseBalance(totals, account, accountUser, principalValue(newBalance));

        // Reserves are decreased by increasing total supply and decreasing borrows
        //  the amount of debt repaid by reserves is `newBalance - oldBalance`
        // Note: new balance must be non-negative due to the above thresholding
        totals.totalSupplyBase += principalValueSupply(unsigned104(newBalance));
        // Note: old balance must be negative since the account is liquidatable
        totals.totalBorrowBase -= principalValueBorrow(unsigned104(-oldBalance));
