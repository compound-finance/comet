...
rule reversability_of_packing(uint8 i, address assetArg, address priceFeedArg, uint8 decimalsArg, uint64 borrowCollateralFactorArg, uint64 liquidateCollateralFactorArg, uint64 liquidationFactorArg, uint128 supplyCapArg){
    ...
    require word_a == get_asset00_a() && word_b == get_asset00_b(); // assumtion that assetXX_a, assetXX_b are being loaded with correct value
    if (assetArg == 0 ){
        ...
        assert (scale_ == exponent_of_ten(decimalsArg), "decimals got packed/unpacked wrongfully");
