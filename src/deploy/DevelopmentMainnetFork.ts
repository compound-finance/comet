...
export async function deployDevelopmentComet(
  ...
  //updatedRoots.set('usdc', USDC);
  // updatedRoots.set('dai', DAI);
  comet = await deploymentManager.contract('comet:implementation') as CometInterface;
  await deploymentManager.putRoots(updatedRoots);

  console.log("updated roots = ", updatedRoots);
  console.log("after roots");
  await deploymentManager.spider();

  console.log("Comet = ", comet.address);
  console.log("Comet proxy = ", cometProxy.address);
  console.log("configuration proxy = ", configuratorProxy.address);
