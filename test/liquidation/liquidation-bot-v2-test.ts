...
describe.only('LiquidatorV2', function () {
  before(forkMainnet);
  ...
  it('Should execute DAI flash swap with profit', async () => {
    ...
    const {
      ...
      swapTargets,
      swapCallDatas
    } = await getSwapInfo(comet, liquidator, [underwater.address]);

    const tx = await wait(
      liquidator.connect(owner).absorbAndArbitrage(
        comet.address,
        [underwater.address],
        swapAssets,
        swapTargets,
  ...
  it('Should execute WETH flash swap with profit', async () => {
    const { comet, liquidator, users: [owner, underwater, recipient], assets: { dai, usdc, weth } } = await makeLiquidatableProtocol();
  ...
  it('Should execute WBTC flash swap with profit', async () => {
    const { comet, liquidator, users: [owner, underwater, recipient], assets: { dai, usdc, wbtc } } = await makeLiquidatableProtocol();
  ...
  it('Should execute UNI flash swap with profit', async () => {
    const { comet, liquidator, users: [owner, underwater, recipient], assets: { dai, usdc, uni } } = await makeLiquidatableProtocol();
  ...
  it('Should execute COMP flash swap with profit', async () => {
    const { comet, liquidator, users: [owner, underwater, recipient], assets: { dai, usdc, comp } } = await makeLiquidatableProtocol();
  ...
  it('Should execute LINK flash swap with profit', async () => {
    const { comet, liquidator, users: [owner, underwater, recipient], assets: { dai, usdc, link } } = await makeLiquidatableProtocol();
  ...
  it('setAssetConfig reverts if called by non-admin', async () => {
    const { comet, liquidator, users: [_signer, underwater], assets: { link } } = await makeLiquidatableProtocol();

    await expect(
      liquidator.connect(underwater).setAssetConfig(comet.address, link.address, exp(150, 18), true)
    ).to.be.revertedWith("custom error 'Unauthorized()'");
  });

  it('setAssetConfig updates asset config', async () => {
    const { comet, liquidator, users: [signer, underwater], assets: { link } } = await makeLiquidatableProtocol();

    const assetConfig = await liquidator.connect(signer).assetConfigs(comet.address, link.address);

    expect(assetConfig.maxCollateralToPurchase).to.eq(0);
    expect(assetConfig.isSet).to.be.false;

    await liquidator.connect(signer).setAssetConfig(
      comet.address,
      link.address,
      exp(250, 18),
      true
    ...
    const updatedAssetConfig = await liquidator.connect(signer).assetConfigs(comet.address, link.address);
