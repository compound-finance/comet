...
describe('buyCollateral', function () {
  ...
  it('reverts if reserves are above target reserves', async () => {
    ...
    // Alice buys 50e18 wei COMP for 50e6 wei USDC
    await wait(baseAsA.approve(comet.address, exp(50, 6)));
    await expect(cometAsA.buyCollateral(COMP.address, exp(50, 18), 50e6, alice.address)).to.be.revertedWith('not for sale');
  });
  ...
  it('reverts if slippage is too high', async () => {
    ...
    await wait(baseAsA.approve(comet.address, exp(50, 6)));
    await expect(cometAsA.buyCollateral(COMP.address, exp(100, 18), 50e6, alice.address)).to.be.revertedWith('too much slippage');
  });
  ...
  it('reverts if buy is paused', async () => {
    ...
    await expect(cometAsA.buyCollateral(COMP.address, exp(50, 18), 50e6, alice.address)).to.be.revertedWith('paused');
  });
