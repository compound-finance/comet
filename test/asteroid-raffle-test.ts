...
describe('AsteroidRaffle', function () {
  beforeEach(async () => {
    ...
    const tx = await raffle.connect(governor).initialize(ticketPrice, raffleDuration);
    await tx.wait();
  });
  ...
  it('Should start, finish and restart raffle', async function () {
    ...
    // Restart raffle
    const newTicketPrice = ethers.utils.parseEther('0.2');
    const tx6 = await raffle.connect(governor).restartRaffle(newTicketPrice, raffleDuration);
    await tx6.wait();
