...
describe('withdrawReserves', function () {
  ...
  it('reverts if not enough reserves are owned by protocol', async () => {
    ...
    await expect(comet.connect(governor).withdrawReserves(user.address, tokenBalance + 1)).to.be.revertedWith('VM Exception while processing transaction: reverted with panic code 0x11 (Arithmetic operation underflowed or overflowed outside of an unchecked block)');
  });
