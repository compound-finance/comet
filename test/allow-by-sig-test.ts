...
describe('allowBySig', function () {
  ...
  it.only('allows manager to authorize zero address', async () => {
    ...
    // authorizes manager for address(0)
    expect(await comet.isAllowed(ethers.constants.AddressZero, manager.address)).to.be.true;
  });
});
