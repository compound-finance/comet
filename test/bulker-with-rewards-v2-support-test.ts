import { baseBalanceOf, ethers, expect, exp, makeProtocol, wait, makeBulkerWithRewardsV2Support, defaultAssets, getGasUsed, makeRewards, fastForward, event, makeRewardsV2, getProof, getRewardsOwed } from './helpers';
...
describe('bulker with rewards v2 support', function () {
  it('supply base asset', async () => {
    const protocol = await makeProtocol({});
    const { comet, tokens: { USDC, WETH }, users: [alice] } = protocol;
    const bulkerInfo = await makeBulkerWithRewardsV2Support({ weth: WETH.address });
  ...
  describe('admin functions', function () {
    it('transferAdmin', async () => {
      ...
      const bulkerInfo = await makeBulkerWithRewardsV2Support({ admin: governor, weth: WETH.address });
      const { bulker } = bulkerInfo;
  ...
  it('should claim rewards v2', async () => {
    const {
      comet,
      governor,
      tokens: { USDC, COMP, WETH },
    ...
    const claimRewardCalldata = ethers.utils.defaultAbiCoder.encode(
      ...
      [comet.address, rewardsV2.address, 0, alice.address, true, [
        // startIndex: 1,
        // finishIndex: 0,
        // startAccrued: 100,
        // finishAccrued: 0,
    ...
    await wait(await bulker.connect(alice).invoke(
    ...
    expect(await COMP.balanceOf(alice.address)).to.be.equal(expectedRewards[0]);
    expect(await USDC.balanceOf(alice.address)).to.be.equal(expectedRewards[1]);
  });

  it('should claim rewards v2 for new member', async () => {
    const {
      comet,
      governor,
      tokens: { USDC, COMP, WETH },
    ...
    const claimRewardCalldata = ethers.utils.defaultAbiCoder.encode(
      [
        ...
        'bool',
        'address[2]',
        '(uint256,uint256,uint256,uint256,bytes32[],bytes32[])[2]',
    ...
    await wait(bulker.connect(charlie).invoke(
      [await bulker.ACTION_CLAIM_REWARD_V2_NEW_MEMBER()],
      [claimRewardCalldata]
    ));

    expect(await COMP.balanceOf(bob.address)).to.be.equal(exp(86401, 18));
    expect(await USDC.balanceOf(bob.address)).to.be.equal(exp(86401, 6));
  });
});
