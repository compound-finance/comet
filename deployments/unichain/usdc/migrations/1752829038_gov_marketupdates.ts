...
export default migration('1752829038_gov_marketupdates', {
  ...
  enact: async (
    ...
    const description = `#Alternate Governance track for Market Updates - Unichain\n\n##Proposal summary\n
WOOF! proposes an alternate governance track for market updates on the Compound III markets on Unichain, following the successful deployment by DoDAO on the Optimism network. This proposal aims to streamline market parameter updates by introducing a parallel governance process, reducing friction, and enabling faster updates while maintaining community oversight.
...
This proposal executes a single cross-chain action via Unichain's 'sendMessage', which forwards encoded instructions to the 'bridgeReceiver' on Unichain. The payload contains multiple actions that are executed on L2:

1. Updating the proxy admin for the USDC and WETH Comet markets to a new 'CometProxyAdmin' contract.
...
All these actions can be executed on Unichain after this relay message is processed.
`;
