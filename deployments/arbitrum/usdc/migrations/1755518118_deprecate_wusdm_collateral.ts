...
export default migration('1755518118_deprecate_wusdm_collateral', {
  async prepare(
  ...
  enact: async (deploymentManager: DeploymentManager, govDeploymentManager: DeploymentManager, { wUSDMPriceFeedAddress }) => {
    ...
    const description = `# Deprecate wUSDM from cUSDCv3 on Arbitrum
## Proposal summary
WOOF! proposes to deprecate wUSDM from cUSDCv3 on Arbitrum network, since deprecation of USDM itself and its Chainlink oracle.
In order to achieve this price feed will be updated to a new one, which will return the smallest acceptable price - 0.00000001 (1e-8), and the supply cup will be set to 0 to prevent further deposits. This proposal takes the governance steps recommended and necessary to update a Compound III USDC market on Arbitrum. Simulations have confirmed the market’s readiness, as much as possible, using the [Comet scenario suite](https://github.com/compound-finance/comet/tree/main/scenario).
