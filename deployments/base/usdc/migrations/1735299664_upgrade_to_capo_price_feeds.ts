...
let oldWstETHToUSDCPriceFeed: string;
...
export default migration('1735299664_upgrade_to_capo_price_feeds', {
  ...
  async enact(deploymentManager: DeploymentManager, govDeploymentManager, {
    ...
    [,, oldWstETHToUSDCPriceFeed] = await comet.getAssetInfoByAddress(WSTETH_ADDRESS);
  ...
  async verify(deploymentManager: DeploymentManager) {
      ...
      expect(await comet.getPrice(newWstETHToUSDCPriceFeed)).to.be.closeTo(await comet.getPrice(oldWstETHToUSDCPriceFeed), 40e8);
    },
});
