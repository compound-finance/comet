...
export default migration('1674835242_bump_supply_caps', {
  ...
  async enact(deploymentManager: DeploymentManager) {
    ...
    const actions = [
      // 1. Increase supply caps for each of the assets
      {
        ...
        args: [comet.address, cbETH.address, exp(20_000, 18)],
      },
    ...
    const description = "# Increase cbETH Supply Cap in cWETHv3\n\n## Explanation\n\nThe cWETHv3 market is currently limited by the cbETH supply cap, which has been reached.\n\nThe associated forum post for this proposal can be found [here](https://www.comp.xyz/t/compound-v3-weth-comet-risk-parameter-updates-2023-01-26).\n\n## Proposal\n\nThe proposal itself is to be made from [this pull request](https://github.com/compound-finance/comet/pull/667).\n\nThe first action of the proposal sets the configurator supply cap for cbETH to 20,000 from the current cap of 7,100.\n\nThe second action deploys and upgrades to a new implementation of Comet, using the newly configured parameters.";
  ...
  async verify(deploymentManager: DeploymentManager) {
    ...
    expect(await cbETHInfo.supplyCap).to.be.eq(exp(20_000, 18));
  },
});
