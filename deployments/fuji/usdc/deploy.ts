...
import { DeploySpec, debug, deployComet, exp, sameAddress, wait } from '../../../src/deploy';
...
async function deployContracts(deploymentManager: DeploymentManager, deploySpec: DeploySpec): Promise<Deployed> {
  ...
  const WBTC = await deploymentManager.clone('WBTC.e', cloneAddr.wbtc, [], cloneNetwork);
  const WAVAX = await deploymentManager.clone('WAVAX', cloneAddr.wavax, [], cloneNetwork);
