...
const CONSTANT_PRICE_FEED = '0x5D173813B4505701e79E654b36A95E6c1FAD4448';

export default migration('1772817323_deprecate_stmatic', {
  async prepare() {
    return {};
  },
  ...
  async enact(deploymentManager: DeploymentManager, govDeploymentManager: DeploymentManager) {
    ...
    const description = `# Deprecate stMATIC in cUSDTv3 on Polygon

## Proposal summary

WOOF! propose to deprecate stMATIC as collateral in cUSDTv3 on Polygon by updating its price feed to a constant price feed with a price of 1 wei and set its supply cap to 0.

Further detailed information can be found on the corresponding [proposal pull request](https://github.com/compound-finance/comet/pull/1095) and [forum discussion](https://www.comp.xyz/t/gauntlet-depreciating-stmatic-on-polygon-usdt-and-usdc-e-comets/7083).
...
`;
  ...
  async verify(deploymentManager: DeploymentManager) {
    ...
    expect(stMaticInCometInfo.priceFeed).to.eq(CONSTANT_PRICE_FEED);
    expect(stMaticInConfiguratorInfo.priceFeed).to.eq(CONSTANT_PRICE_FEED);
    expect(await comet.getPrice(CONSTANT_PRICE_FEED)).to.equal(1);
