...
export default migration('1739783281_configurate_and_ens', {
  ...
  enact: async (
    ...
    const _depositUSDCCalldata = await sonicL1GatewayBridge.populateTransaction.deposit(uid, USDC_L1.address, USDCAmountToSeed);
    ...
    const actions = [
      ...
      //   args: [sonicL1GatewayBridge.address, COMPAmountToBridge],
      // },
      // // 2. Bridge COMP from Ethereum to Unichain Rewards using L1StandardBridge
      // {
      //   contract: sonicL1GatewayBridge,
      //   signature: 'deposit(uint96,address,uint256)',
      //   args: [Date.now(), COMP_L1.address, COMPAmountToBridge],
      // },
      ...
      // 4. Bridge USDC to Sonic via CCTP
      {
        target: delegatorAddress,
        signature: 'call(address[],bytes[],uint256[])',
        calldata: depositUSDCCalldata
        // args: [
        //   [USDC_L1.address, sonicL1GatewayBridge.address],
        //   [_approveUSDCCalldata.data, _depositUSDCCalldata.data],
        //   [0, 0]
        // ]
      },
  ...
  async verify(
    ...
    const depositId = await l2SonicBridge.userOperationId(delegatorAddress, uid);

    const storageSlot = utils.keccak256(
      utils.defaultAbiCoder.encode(['uint256', 'uint8'], [depositId, 7])
    );

    const dataFromStorage = await govDeploymentManager.hre.ethers.provider.getStorageAt(
      sonicL1GatewayBridge.address,
      storageSlot
    );
    expect(dataFromStorage).to.not.be.equal(constants.HashZero);
