...
export default async function deploy(deploymentManager: DeploymentManager, deploySpec: DeploySpec): Promise<Deployed> {
  ...
  const COMP = await deploymentManager.clone('COMP', COMP_ADDRESS, [admin.address], cloneNetwork);
  ...
  await deploymentManager.idempotent(
    ...
    async () => {
      debug(`Sending half of all COMP to `);
      const amount = (await COMP.balanceOf(admin.address));
      await wait(COMP.connect(admin).transfer(admin.address, amount));
