...
const pauseGuardianAddress = '0x2127338F0ff71Ecc779dce407D95C7D32f7C5F45';
...
export default migration('1752829069_gov_marketupdates', {
  prepare: async () => {
    return {};
  },
  ...
  enact: async (
    ...
    const description = `#Alternate Governance track for Market Updates - Mantle\n\n##Proposal summary\n
WOOF! proposes an alternate governance track for market updates on the Compound III markets on Mantle, following the successful deployment by DoDAO on the Optimism network. This proposal aims to streamline market parameter updates by introducing a parallel governance process, reducing friction, and enabling faster updates while maintaining community oversight.
...
This proposal executes a single cross-chain action via Mantle's 'sendMessage', which forwards encoded instructions to the 'bridgeReceiver' on Mantle. The payload contains multiple actions that are executed on L2:

1. Updating the proxy admin for the USDe Comet market to a new 'CometProxyAdmin' contract.
...
All these actions can be executed on Mantle after this relay message is processed.
`;
