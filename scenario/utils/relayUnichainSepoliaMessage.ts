...
export default async function relayUnichainSepoliaMessage(
  governanceDeploymentManager: DeploymentManager,
  bridgeDeploymentManager: DeploymentManager,
  startingBlockNumber: number
) {
  const unichainSepoliaL1CrossDomainMessenger = await governanceDeploymentManager.getContractOrThrow('unichainSepoliaL1CrossDomainMessenger');
  ...
  const filter = unichainSepoliaL1CrossDomainMessenger.filters.SentMessage();
  const sentMessageEvents: Log[] = await governanceDeploymentManager.hre.ethers.provider.getLogs({
    fromBlock: startingBlockNumber,
    toBlock: 'latest',
    address: unichainSepoliaL1CrossDomainMessenger.address,
    topics: filter.topics!
  });

  for (let sentMessageEvent of sentMessageEvents) {
    const { args: { target, sender, message, messageNonce, gasLimit } } = unichainSepoliaL1CrossDomainMessenger.interface.parseLog(sentMessageEvent);
    const aliasedSigner = await impersonateAddress(
      bridgeDeploymentManager,
      applyL1ToL2Alias(unichainSepoliaL1CrossDomainMessenger.address)
    );
    ...
    if (target === l2StandardBridge.address) {
      ...
      try {
        ...
        // This is our best attempt to mimic the deposit transaction type (not supported in Hardhat) that Unichain-Sepolia uses to deposit ETH to an L2 address
