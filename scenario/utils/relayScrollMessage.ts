...
export default async function relayScrollMessage(
  ...
  const filter = scrollMessenger.filters.MessageSent();
  ...
  for (let messageSentEvent of messageSentEvents) {
    ...
    await l2Messenger.connect(aliasSetterRoleAccount).addL1L2MessageHashes([_messageHash]);

    const relayMessageTxn = await (
      await l2Messenger.claimMessage(
        _from,
        _to,
        _fee,
        _value,
