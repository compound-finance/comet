...
import { COMP_WHALES, proposal } from "../../src/deploy";
...
export async function voteForOpenProposal(
  ...
  if (blocksUntilEnd > 0) {
    if (tenderly) {
      try {
        const voter = (await dm.getSigners())[1];
        await governor.connect(voter).castVote(id, 1);
      } catch (err) {
        debug(`Error while voting tenderly`, err.message);
      }
    } else {
