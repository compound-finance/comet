...
export async function tenderlyExecute(
  ...
  // 2. Build a governance timeline (5 blocks total)
  //    B0   : deploy + propose
  //    B0+1 : proposal becomes Active
  //    B0+2 : whales cast votes → succeeds
  //    B0+3 : queue
  //    B0+6 : execute (Timelock.delay is patched to 0)
  ...
  // 3. Gather cached proposal args and common interfaces
  const proposalArgs = loadCachedProposal();
  proposalArgs.pop(); // drop signatures (empty)

  const id   = BigInt(await governor.proposalCount()); // next proposal id
  ...
  // Remove possible leading undefined entries (no deployments)
  while (!sims[0]) sims.shift();

  // 6. Run the bundle simulation on Tenderly
