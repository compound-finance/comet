  ...
  for (const log of logsCCIP) {
    ...
    if (internalMsg.tokenAmounts.length) {
      for (const tokenTransferData of internalMsg.tokenAmounts) {
        ...
        if (mintAmount.lte(0)) {
          console.log(`[CCIP L1->L2] No mint needed for ${tokenTransferData.token}`);
          const transferTx = await l2Token.connect(signer).transfer(internalMsg.receiver, tokenTransferData.amount);
          await transferTx.wait();
          console.log(`[CCIP L1->L2] Transferred ${tokenTransferData.amount.toString()} of ${tokenTransferData.token} to ${internalMsg.receiver}`);
        } else {
          console.log(`[CCIP L1->L2] Minting ${mintAmount.toString()} of ${tokenTransferData.token} to ${internalMsg.receiver}`);
          const mintTx = await l2Token.connect(signer).mint(internalMsg.receiver, mintAmount);
          await mintTx.wait();
          console.log(`[CCIP L1->L2] Minted ${mintAmount.toString()} of ${tokenTransferData.token} to ${internalMsg.receiver}`);
        }
      }
    }
