  ...
  async solve() {
    return async function (ctx: T): Promise<T> {
      ...
      for (const proposal of proposals) {
        ...
        // temporary hack to skip proposal 427
        if (proposal.id.eq(427)) {
          console.log('Skipping proposal 427');
          continue;
        }

        // Temporary hack to skip proposals 510, 511, and 512
        if (proposal.id.eq(512) || proposal.id.eq(510) || proposal.id.eq(511)) {
          console.log('Skipping proposal 510, 511, and 512');
          continue;
        }
